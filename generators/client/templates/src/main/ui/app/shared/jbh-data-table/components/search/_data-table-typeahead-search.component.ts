...
@Component({
  selector: 'datatable-typeahead-search',
  template: `
    <div class="typeahead">
      <input #inputElement
        class="form-control"
...
export class DataTableTypeAheadSearchComponent implements OnInit, ControlValueAccessor {
  abstract;

  constructor (private http: Http) {}

  private getSuggestions(): any {
    return this.http.get(this.lookupService+this.input).subscribe(res => this.extractData(res))
  }

  private extractData(res: Response) {
    let body = res.json();
    let addressList:any[] = body.Results || { };
    let addresses:Address[] = [];

    for(let i = 0;i<addressList.length;i++){
      let address:Address = new Address();
      address.addressLine1=addressList[i]['Address']['AddressLine1'];
      address.city=addressList[i]['Address']['City'];
      address.state=addressList[i]['Address']['State'];
      address.zipCode=addressList[i]['Address']['PostalCode'];
      address.fullAddress=address.addressLine1+' '+address.city+' '+address.state+' '+address.zipCode;
   ...
   */
  @Input() searchProperty:string = 'fullAddress';

  /**
   * The property of a list item that should be displayed.
   */
  @Input() displayProperty:string = 'fullAddress';
  ...
  public populateSuggestions() {
    ...
    //let input = this.inputElement.nativeElement.value;

    //let input = this.inputElement.text;
    ...
    // this.suggestions = this.findSuggestions().filter(function (item) {
    //   if(searchProperty){
